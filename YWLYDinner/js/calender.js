...
function fDrawCal(iYear, iMonth, iCellHeight, iDateTextSize) {
	...
	var styleTD = " bgcolor='" + gcBG + "' bordercolor='" + gcBG
			...
			+ "' style='font:bold " + iDateTextSize + " 宋体;color:#FFFFFF;";
	with (document) {
		write("<tr>");
		for (i = 0; i < 7; i++)
			if (i == 0) {
				write("<td " + styleTD + "background-color:#EB9797;' >"
						+ WeekDay[i] + "</td>");
			} else if (i == 6) {
				write("<td " + styleTD + "background-color:#EB9797;' >"
						+ WeekDay[i] + "</td>");
			} else
				write("<td " + styleTD + "background-color:#C0C0C0;' >"
						+ WeekDay[i] + "</td>");
		write("</tr>");
		for (w = 1; w < 7; w++) {
			write("<tr>");
...
function fTagInBound(aTag) {
	with (VicPopCal.style) {
		...
		return !((ptLT.x > r) || (ptLT.x + aTag.offsetWidth < l)
				|| (ptLT.y > b) || (ptLT.y + aTag.offsetHeight < t));
	}
}
...
var gMonths = new Array('&nbsp;&nbsp;一月', '&nbsp;&nbsp;二月', '&nbsp;&nbsp;三月',
		'&nbsp;&nbsp;四月', '&nbsp;&nbsp;五月', '&nbsp;&nbsp;六月', '&nbsp;&nbsp;七月',
		'&nbsp;&nbsp;八月', '&nbsp;&nbsp;九月', '&nbsp;&nbsp;十月', '十一月', '十二月');
with (document) {
	...
	write("<tr><td align='center'>");
	write("<b style='cursor:hand;font:bold 14 Default Sans Serif;color:#FFFFFF;' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=yqToggle'>今天："
			+ giYear
			+ "年"
			+ giMonth
			+ "月"
			+ giDay
			+ "日</b><input type=button value=清除 onclick=cleanup() />");
...
function rollDays(theDate, rollDay) {
	...
	var month = strDay.substring(strDay.indexOf("-") + 1, strDay
			.lastIndexOf("-"));
	...
	var solarMonth = null;
	if (isLeapYear(year))
		solarMonth = new Array(0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30,
				31);
	else
		solarMonth = new Array(0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30,
				31);
